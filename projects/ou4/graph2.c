...
graph *graph_insert_edge(graph *g, node *n1, node *n2)
{
	...
	for (int i = 0; i < g->num_nodes; i++)
	{
		...
		if (strcmp(inspection_source_node->label, n1->label) == 0)
		{
			node_source_index = i;
		}
	}
...
{
	dlist *neighbours = dlist_empty(free);
	int node_index = -1;
	...
	for (int i = 0; i < g->num_nodes; i++)
	{
		if (*(int*)array_2d_inspect_value(g->matrix, i, node_index) == 1)
		{
			node *inspection_node = array_1d_inspect_value(g->nodes, i);
			if (strcmp(inspection_node->label, n->label) != 0)
			{
				dlist_pos pos = dlist_first(neighbours);
				dlist_insert(neighbours, inspection_node->label, pos);
			}
		}
	}
	return neighbours;
...
bool graph_node_is_seen(const graph *g, node *n)
{
	return n->seen;
}
